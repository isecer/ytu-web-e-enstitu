    ...
    public static class MezuniyetBus
    {
        ...
        public static void TezDosyasiKontrolYetkilisiAta(int mezuniyetBasvurulariId)
        {
            using (var db = new LisansustuBasvuruSistemiEntities())
            {
                ...
                if (basvuru.MezuniyetBasvurulariTezDosyalaris.Count > 1 || basvuru.TezKontrolKullaniciID.HasValue) return;
                var groupToplamAtamaList =
                    (from kul in db.Kullanicilars.Where(p =>
                            p.YetkiGrupID == 13 && p.IsAktif && p.EnstituKod == basvuru.MezuniyetSureci.EnstituKod)
                     join mez in db.MezuniyetBasvurularis.Where(p =>
                             p.TezKontrolKullaniciID.HasValue &&
                             p.MezuniyetSureci.EnstituKod == basvuru.MezuniyetSureci.EnstituKod) on kul.KullaniciID
        ...
        public static MezuniyetBasvuruDetayDto GetMezuniyetBasvuruDetayBilgi(int mezuniyetBasvurulariId, int? mezuniyetBasvurulariYayinId = null, int? showDetayYayinId = null)
        {
            ...
            using (var db = new LisansustuBasvuruSistemiEntities())
            {
                ...
                model.MezuniyetBasvurulariTezDosyalariDtos = basvuru.MezuniyetBasvurulariTezDosyalaris.Select(s => new MezuniyetBasvurulariTezDosyalariDto
                {
                    ...
                    TezKontrolYetkiliAdSoyad = tezAtananKullanici != null ? tezAtananKullanici.Ad + " " + tezAtananKullanici.Soyad : null
                }).ToList();
                ...
                model.MezuniyetSRModel.SalonRezervasyonlari = (from s in db.SRTalepleris
                                                               ...
                                                               select new FrTalepler
                                                               {
                                                                   ...
                                                                   UzatmaTaahhutSonTarih = s.MezuniyetSinavDurumID == MezuniyetSinavDurumEnum.Uzatma ?
                                                                                                          s.UzatmaSonrasiOgrenciTaahhutSonTarih ?? (EntityFunctions.AddDays(s.Tarih, bSurecOtKriter.MBSinavUzatmaOgrenciTaahhutMaxGun).Value)
                                                                                                          : DateTime.Now,
                                                                   ...
                                                                   UzatmaSonSrTarih = s.MezuniyetSinavDurumID == MezuniyetSinavDurumEnum.Uzatma ?
                                                                                                s.UzatmaSonrasiYeniSinavTalebiSonTarih ?? EntityFunctions.AddDays(s.Tarih, bSurecOtKriter.MBSinavUzatmaSinavAlmaSuresiMaxGun).Value
                                                                                                : DateTime.Now,
        ...
        public static List<CmbIntDto> GetCmbAktifTezKontrolSorumlulari(bool bosSecimVar = false, bool tumu = false)
        {
            var dct = new List<CmbIntDto>();
            if (bosSecimVar) dct.Add(new CmbIntDto { Value = null, Caption = "" });
            using (var db = new LisansustuBasvuruSistemiEntities())
            {
                var data = db.Kullanicilars.Where(p => p.IsAktif && p.YetkiGrupID == 13).OrderBy(o => o.Ad).ThenBy(t => t.Soyad).Select(s => new { s.KullaniciID, s.Ad, s.Soyad }).ToList();
                foreach (var item in data)
                {
        ...
        public static MmMessage SendMailMezuniyetDegerlendirmeLink(int srTalepId, Guid? uniqueId = null, bool isLinkOrSonuc = false, bool isYeniLink = true, string eMail = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                SistemBilgilendirmeBus.SistemBilgisiKaydet(message + "\r\n Hata:" + ex.ToExceptionMessage(), "MezuniyetBus/sendMailMezuniyetDegerlendirmeLink \r\n" + ex.ToExceptionStackTrace(), LogTipiEnum.Hata);
        ...
        public static MmMessage SendMailMezuniyetTezSablonKontrol(int mezuniyetBasvurulariTezDosyaId, int sablonTipId, string aciklama = "")
        {
            ...
            try
            {
                using (var db = new LisansustuBasvuruSistemiEntities())
                {
                    ...
                    var sablonTipleri = db.MailSablonTipleris.Where(p => sablonTipIds.Contains(p.MailSablonTipID))
                        .ToList();
                    var sablonlar = db.MailSablonlaris.Where(p => sablonTipIds.Contains(p.MailSablonTipID)).ToList();
                    foreach (var item in mModel)
                    {
            ...
            catch (Exception ex)
            {
                ...
                SistemBilgilendirmeBus.SistemBilgisiKaydet(msg + "\r\n Hata:" + ex.ToExceptionMessage(), "MezuniyetBus/SendMailMezuniyetTezSablonKontrol \r\n" + ex.ToExceptionStackTrace(), LogTipiEnum.Hata);
