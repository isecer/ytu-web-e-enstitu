...
namespace LisansUstuBasvuruSistemi.Controllers
{
    ...
    [Authorize]
    public class YYDBasvuruController : Controller
    {
        ...
        public ActionResult BasvuruYap(int? BasvuruID, int? KullaniciID = null, string EnstituKod = "", string EKD = "")
        {
            ...
            _MmMessage = Management.getAktifBasvurSurecKontrol(model.EnstituKod, BasvuruSurecTipi.YTUYeniMezunDRBasvuru, KullaniciID, BasvuruID);



            var kul = db.Kullanicilars.Where(p => p.KullaniciID == KullaniciID).FirstOrDefault();
            if (_MmMessage.IsSuccess && !BasvuruID.HasValue && EnstituKodlari.SosyalBilimleri != model.EnstituKod)
            {
                model.BasvuruSurecID = Management.getAktifBasvuruSurecID(model.EnstituKod, BasvuruSurecTipi.YTUYeniMezunDRBasvuru).Value;
                var surec = db.BasvuruSurecs.Where(p => p.BasvuruSurecID == model.BasvuruSurecID).First();
                var KriterUygun = db.MezuniyetBasvurularis.Any(p => p.KullaniciID == KullaniciID && p.IsMezunOldu == true && p.MezuniyetTarihi >= surec.SonucGirisBaslangicTarihi.Value);
                if (!KriterUygun)
                {
                    _MmMessage.IsSuccess = false;
                    _MmMessage.Messages.Add("Başvuru yapabilmeniz için mezuniyet başvurunuzun olması ve mezuniyet tarihinizin '" + surec.SonucGirisBaslangicTarihi.Value.Date.ToDateString() + "' tarihine eşit yada daha sonrası olması gerekmektedir. ");
                }
            }
        ...
        public ActionResult BasvuruYap(kmBasvuru kModel)
        {
            ...
            else
            {
                ...
                foreach (var item in qgrup.Where(p => p.Count > 1))
                {
                    ...
                    _MmMessage.Messages.Add("Bir Başvuruda Bir Program Bir Kere Seçilebilir.");
                ...
                if (succes)
                {
                    ...
                    foreach (var item in qtercih)
                    {
                        ...
                        {
                            _MmMessage.Messages.Add("Seçilen Program İin Kota bulunmamaktadır.");
                            succes = false;
                        }
            ...
            if (_MmMessage.Messages.Count == 0)
            {
                ...
                if (sendMail && bsurec.Enstituler.LUBMailGonder)
                {
                    ...
                    mtc.Detaylar.Add(new mailTableRow { Baslik = "Başvuru Türü", Aciklama = "YTU Yeni Mezun Doktora Başvurusu" });
        ...
        public ActionResult getOTdetay(int BasvuruSurecID, int otID, int KullaniciID, int? BasvuruID = null, List<int> ArrOtipIds = null)
        {
            ...
            var mdl = Management.getOgrenimTipiKotaBilgi( BasvuruSurecID, otID, KullaniciID, BasvuruSurecTipi.YTUYeniMezunDRBasvuru, BasvuruID, ArrOtipIds);
