...
namespace LisansUstuBasvuruSistemi.Controllers
{
    ...
    public class TIAyarlarController : Controller
    {
        ...
        { 
            ...
            var data = db.TIAyarlars.Where(p => p.EnstituKod == _EnstituKod && UserIdentity.Current.EnstituKods.Contains(p.EnstituKod)).OrderBy(o => o.Kategori).ThenBy(t => t.SiraNo).ToList();
        ...
        public ActionResult Index(List<string> AyarAdi, List<string> AyarDegeri, List<string> PanelToggled, string EKD)
        {
            ...
            {
                var ayar = db.TIAyarlars.Where(p => p.AyarAdi == item.AyarAdi && p.EnstituKod == _EnstituKod).FirstOrDefault();
                if (ayar != null)
                {
