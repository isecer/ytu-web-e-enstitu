...
namespace LisansUstuBasvuruSistemi.Controllers
{
    ...
    public class TiBasvuruController : Controller
    {
        ...
        public ActionResult Index(FmTiBasvuru model, string ekd)
        {
            ...
            {
                bbModel.SistemBasvuruyaAcik = TiAyar.TiBasvurusuAcikmi.GetAyar(enstituKod, "false").ToBoolean(false);
        ...
        public ActionResult GetTiAraRaporFormu(int tiBasvuruId, int? tiBasvuruAraRaporId)
        {
            ...
            else if (tiBasvuruAraRapor == null && TiAyar.TiBasvurusuAcikmi.GetAyar(tiBasvuru.EnstituKod).ToBoolean() == false)
            {
            ...
            else
            {
                ...
                {
                    var sondonemKayitolmasiGerekenDersKodlari = TiAyar.TiSonDonemKayitOlunmasiGerekenDersKodlari.GetAyar(tiBasvuru.EnstituKod);
        ...
        public ActionResult TiAraRaporFormuPost(TiAraRaporFormuModel kModel, string donemSecim, bool saveData = false)
        {
            ...
            else
            {
                ...
                var kayitYapilacakDersKodlaris = isYeniJo ? TiAyar.TiSonDonemKayitOlunmasiGerekenDersKodlari.GetAyar(tiBasvuru.EnstituKod).Split(',').Where(p => p.Trim() != "").Select(s => s.Trim()).ToList() : new List<string>();
                ...
                if (mMessage.Messages.Count == 0)
                {
                    ...
                    if (mMessage.Messages.Count == 0 && saveData)
                    {
                        ...
                        var juriUyeleri = qData
                            .Where(d => !string.IsNullOrWhiteSpace(d.Row.AdSoyad) && !string.IsNullOrWhiteSpace(d.Row.UnvanAdi))
                            .Select(d => new {
                                AdSoyad = d.Row.AdSoyad.Trim().ToUpper(),
                                UnvanAdi = d.Row.UnvanAdi.Trim().ToUpper(),
                                EMail = d.Row.EMail?.Trim().ToLower(),
                                JuriTipAdi = d.Row.JuriTipAdi,
                                KisiKimlik = $"{d.Row.AdSoyad.Trim().ToUpper()} - {d.Row.UnvanAdi.Trim().ToUpper()}"
                            })
                            .ToList();
                        ...
                        {
                            var juriTipAdlari = string.Join(", ", mukerrerKisi.JuriTipleri);
                        ...
                        if (mukerrerKisiler.Any())
                        {
                            if (errSelectedTabId == 0)
                            {
                                ...
                                {
                                    var ilkMukerrerItem = qData.FirstOrDefault(q => q.Row.JuriTipAdi == ilkMukerrer);
                                    if (ilkMukerrerItem != null)
                                    {
                                        kModel.SelectedTabId = ilkMukerrerItem.Row.TabID;
                                    }
                                }
                            }
                        }
                    ...
                    if (mMessage.Messages.Count == 0 && saveData)
                    {
                        try
                        {
                            ...
                            if (isYeniJo)
                            {
                                ...
                                tiBasvuruAraRapor.BasvuruSonDonemSecilecekDersKodlari = TiAyar.TiSonDonemKayitOlunmasiGerekenDersKodlari.GetAyar(tiBasvuru.EnstituKod);
        ...
        public ActionResult RezervasyonAlPost(SrTalepleriKayitDto kModel, bool isSendMail = true)
        {
            ...
            else
            {
                if (kModel.IsOnline == true && !TiAyar.TiAraRaporSinaviOnlineYapilabilsin
                        .GetAyar(kModel.EnstituKod).ToBoolean(false))
                {
