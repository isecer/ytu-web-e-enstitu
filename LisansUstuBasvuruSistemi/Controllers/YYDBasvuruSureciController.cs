...
namespace LisansUstuBasvuruSistemi.Controllers
{
    ...
    public class YYDBasvuruSureciController : Controller
    {
        ...
        public ActionResult Index(fmBasvuruSureci model)
        {
            ...
            var q = from s in db.BasvuruSurecs.Where(p => p.BasvuruSurecTipID == BasvuruSurecTipi.YTUYeniMezunDRBasvuru)
        ...
        public ActionResult Kayit(int? id, string dlgid, string EKD)
        {
            ...
            model.OgrenimTipModel.OgrenimTipleriDataList = (from o in db.OgrenimTipleris.Where(p => p.EnstituKod == _EnstituKod && (p.OgrenimTipKod == OgrenimTipi.ButunlesikDoktora || p.OgrenimTipKod == OgrenimTipi.Doktra || p.OgrenimTipKod == OgrenimTipi.SanattaYeterlilik))
                                                            ...
                                                            select new CheckObject<krOgrenimTip>
                                                            {
                                                                Value = new krOgrenimTip
                                                                {
                                                                    ...
                                                                    YokOgrenciKontroluYap = false,
                                                                    IstenecekKatkiPayiTutari = null,
        ...
        public ActionResult Kayit(kmBasvuruSurec kModel, string dlgid = "")
        {
            ...
            kModel.SeciliOgrenimTipKod = kModel.SeciliOgrenimTipKod ?? new List<string>();
            ...
            if (!kModel.SonucGirisBaslangicTarihi.HasValue)
            {
                string msg = "Tez Teslim Formu Tarih Kriteri bilgisini giriniz.";
                MmMessage.Messages.Add(msg);
                MmMessage.MessagesDialog.Add(new MrMessage { MessageType = Msgtype.Warning, PropertyName = "SonucGirisBaslangicTarihi" });
            }
            else MmMessage.MessagesDialog.Add(new MrMessage { MessageType = Msgtype.Success, PropertyName = "SonucGirisBaslangicTarihi" });
            ...
            {
                if (SurecOgrenimTipBilgileri.Any(a => a.Secildi && a.Kota <= 0))
                    MmMessage.Messages.Add("Öğrenim tiplerindeki kota bilgisi boş bırakılamaz ve 0 dan büyük olmalıdır!");
                if (SurecOgrenimTipBilgileri.Any(a => a.Secildi && a.BasariNotOrtalamasi <= 0 || a.BasariNotOrtalamasi > 100))
                ...
                if (kModel.IsBelgeYuklemeVar)
                    if (SurecOgrenimTipBilgileri.Any(a => a.Secildi && !a.AsilBasTar.HasValue) ||
                          SurecOgrenimTipBilgileri.Any(a => a.Secildi && !a.AsilBitTar.HasValue) ||
                          SurecOgrenimTipBilgileri.Any(a => a.Secildi && !a.YedekBasTar.HasValue) ||
                          SurecOgrenimTipBilgileri.Any(a => a.Secildi && !a.YedekBitTar.HasValue))
                        MmMessage.Messages.Add("Öğrenim tiplerindeki belge yükleme tarihleri boş bırakılamaz!");
                    else if (SurecOgrenimTipBilgileri.Any(a => a.Secildi && a.AsilBasTar > a.AsilBitTar) || SurecOgrenimTipBilgileri.Any(a => a.YedekBasTar > a.YedekBitTar))
            ...
            if (MmMessage.Messages.Count == 0)
            {
                ...
                var qBasS = db.BasvuruSurecs.Where(p => p.BasvuruSurecTipID == BasvuruSurecTipi.YTUYeniMezunDRBasvuru && p.EnstituKod == kModel.EnstituKod && p.BasvuruSurecID != kModel.BasvuruSurecID && ((p.BaslangicTarihi <= kModel.BaslangicTarihi && p.BitisTarihi >= kModel.BaslangicTarihi) || (p.BaslangicTarihi <= kModel.BitisTarihi && p.BitisTarihi >= kModel.BitisTarihi))).Count();
                if (qBasS > 0)
                {
            ...
            if (MmMessage.Messages.Count == 0)
            {
                ...
                if (kModel.BasvuruSurecID <= 0)
                {
                    ...
                    tablo = db.BasvuruSurecs.Add(new BasvuruSurec
                    {
                        ...
                        BasvuruSurecTipID = BasvuruSurecTipi.YTUYeniMezunDRBasvuru,
            ...
            kModel.OgrenimTipModel.OgrenimTipleriDataList = (from o in db.OgrenimTipleris.Where(p => p.EnstituKod == kModel.EnstituKod && (p.OgrenimTipKod == OgrenimTipi.ButunlesikDoktora || p.OgrenimTipKod == OgrenimTipi.Doktra || p.OgrenimTipKod == OgrenimTipi.SanattaYeterlilik))
            ...
            {
                var sItem = SurecOgrenimTipBilgileri.Where(p => p.OgrenimTipKod == item.Value.OgrenimTipKod).FirstOrDefault();
                if (sItem != null)
                {
        ...
        public void KBKopyala(int basvuruSurecID, string EnstituKod)
        {
            ...
            foreach (var item in kotalar)
            {
                ...
                var bsst = db.BasvuruSurecKotalars.Add(new BasvuruSurecKotalar
                {
                    ...
                    OrtakKotaSayisi = item.OrtakKotaSayisi > 0 ? 200 : 0,
                    AlanIciKota = item.OrtakKota ? 0 : (item.AlanIciKota > 0 ? 200 : 0),
                    AlanDisiKota = item.OrtakKota ? 0 : (item.AlanDisiKota > 0 ? 200 : 0),
        ...
        public ActionResult getBsSubData(int id, int tbInx, bool IsDelete)
        {
            ...
            if (tbInx == 1)
            {
                ...
                #endregion
                page = Management.RenderPartialView("YYDBasvuruSureci", "getBsDetAnaBilgi", mdl);
            }
            if (tbInx == 2)
            {
                #region SinavTipleri
                ...
                #endregion
                page = Management.RenderPartialView("YYDBasvuruSureci", "getBsSinavTipleri", mdl);
            }
            if (tbInx == 3)
            {
                ...
                #endregion
                page = Management.RenderPartialView("YYDBasvuruSureci", "getBsKotalar", mdl);
            }
            if (tbInx == 4)
            {
                ...
                page = Management.RenderPartialView("YYDBasvuruSureci", "getBsMulakat", mdl);
            }
            if (tbInx == 5)
            {
                ...
                #endregion
                mdl.IsDelete = IsDelete;
                page = Management.RenderPartialView("YYDBasvuruSureci", "getBsSonuc", mdl);
            }
            if (tbInx == 6)
            {
                ...
                #endregion
                mdl.IsDelete = IsDelete;
                page = Management.RenderPartialView("YYDBasvuruSureci", "getBsAnket", mdl);
            }
        ...
        public ActionResult AlanDegistirmeIslemi(string UniqueID, bool IsAlanIciOrDisi)
        {
            ...
            else
            {
                ...
                if (mmMessage.Messages.Count == 0)
                {
                    ...
                    var UpdatedjsonStr = JsonConvert.SerializeObject(new { Tercih.BasvuruTercihID, Tercih.BasvuruID, Tercih.SiraNo, Tercih.OgrenimTipKod, Tercih.ProgramKod, Tercih.AlanTipID });
