...
namespace LisansUstuBasvuruSistemi.Controllers
{
    ...
    [Authorize(Roles = RoleNames.YydKotalar)]
    public class YYDKotalarController : Controller
    {
        ...
        public ActionResult Index(FmKotalarDto model, bool export = false)
        {
            ...
            var q = from k in db.Kotalars.Where(p => p.BasvuruSurecTipID == BasvuruSurecTipi.YTUYeniMezunDRBasvuru)
            ...
            if (!model.Sort.IsNullOrWhiteSpace())  q.OrderBy(model.Sort);
        ...
        public ActionResult Kayit(KmKotalar kModel, string EnstituKod, string AnabilimDaliKod, string dlgid = "")
        {
            ...
            if (MmMessage.Messages.Count == 0)
            {
                ...
                var kt = db.Kotalars.Where(p => p.BasvuruSurecTipID == BasvuruSurecTipi.YTUYeniMezunDRBasvuru && p.KotaID != kModel.KotaID && p.ProgramKod == kModel.ProgramKod && p.OgrenimTipKod == kModel.OgrenimTipKod).FirstOrDefault();
                if (kt != null)
                {
        ...
        public ActionResult getWsData(string EnstituKod)
        {
            ...
            if (userEnst.Contains(EnstituKod))
            {
                try
                {
                    ...
                    var qdata = Management.getWsKotalar(EnstituKod,BasvuruSurecTipi.YTUYeniMezunDRBasvuru);
                    if (qdata.Count > 0)
                    {
                        var kotalar = db.Kotalars.Where(p => p.BasvuruSurecTipID == BasvuruSurecTipi.YTUYeniMezunDRBasvuru && p.EnstituKod == EnstituKod).ToList();
