...
namespace LisansUstuBasvuruSistemi.Controllers
{
    ...
    [Authorize]
    public class YGBasvuruController : Controller
    {
        ...
        public ActionResult Index(fmBasvurular model, string EKD)
        {
            ...
            var q = from s in db.Basvurulars.Where(p => p.KullaniciID == (KullaniciID ?? p.KullaniciID) && p.BasvuruID == (model.BelgeDetailBasvuruID ?? p.BasvuruID))
                    ...
                    join bs in db.BasvuruSurecs.Where(p => p.BasvuruSurecTipID == BasvuruSurecTipi.YatayGecisBasvuru) on s.BasvuruSurecID equals bs.BasvuruSurecID
        ...
        public ActionResult BasvuruYap(int? BasvuruID, int? KullaniciID = null, string EnstituKod = "", string EKD = "")
        {
            ...
            _MmMessage = Management.getAktifBasvurSurecKontrol(model.EnstituKod, BasvuruSurecTipi.YatayGecisBasvuru, KullaniciID, BasvuruID);
            ...
            if (_MmMessage.IsSuccess)
            {
                ...
                {
                    model.BasvuruSurecID = Management.getAktifBasvuruSurecID(model.EnstituKod, BasvuruSurecTipi.YatayGecisBasvuru).Value;
                ...
                ViewBag.DRUniversiteID = new SelectList(Management.cmbGetAktifUniversiteler(true), "Value", "Caption", model.DRUniversiteID);
                ViewBag.DROgrenciBolumID = new SelectList(Management.cmbGetOgrenciBolumleri(model.EnstituKod, true), "Value", "Caption", model.DROgrenciBolumID);
                ViewBag.DRNotSistemID = new SelectList(Management.cmbGetNotSistemleri(true), "Value", "Caption", model.DRNotSistemID);
                ViewBag.OgrenimTipKod = new SelectList(Management.cmbGetAktifOgrenimTipleriGrup(model.BasvuruSurecID, true), "Value", "Caption", "");
                ViewBag.SubOgrenimTipKod = new SelectList(Management.cmbGetAktifSubOgrenimTipleri(model.BasvuruSurecID, "-_O", false), "Value", "Caption", "");
        ...
        public ActionResult BasvuruYap(kmBasvuru kModel)
        {
            ...
            _MmMessage = Management.getAktifBasvurSurecKontrol(kModel.EnstituKod, BasvuruSurecTipi.YatayGecisBasvuru, kModel.KullaniciID, kModel.BasvuruID.toNullIntZero());
            if (kModel.BasvuruID <= 0)
            {
                kModel.BasvuruSurecID = Management.getAktifBasvuruSurecID(kModel.EnstituKod, BasvuruSurecTipi.YatayGecisBasvuru) ?? 0;
            ...
            else
            {
                ...
                if (succes)
                {
                    ...
                    if (kModel.YLDurum) BolumIDs.Add(kModel.DROgrenciBolumID.Value);
            ...
            if (_MmMessage.Messages.Count == 0)
            {
                ...
                if (!kModel.YLDurum)
                {
                    kModel.DRMezuniyetNotu = null;
                    kModel.DRUniversiteID = null;
                    kModel.DRFakulteAdi = null;
                    kModel.DROgrenciBolumID = null;
                    kModel.DRBaslamaTarihi = null;
                    kModel.DRMezuniyetTarihi = null;
                    kModel.DRNotSistemID = null;
                    kModel.DRMezuniyetNotu = null;
                    kModel.DREgitimDiliTurkce = null;
                }
                ...
                if (kModel.BasvuruID <= 0)
                {
                    ...
                    data = db.Basvurulars.Add(new Basvurular
                    {
                        ...
                        DRUniversiteID = kModel.DRUniversiteID,
                        DRFakulteAdi = kModel.DRFakulteAdi,
                        DROgrenciBolumID = kModel.DROgrenciBolumID,
                        DRBaslamaTarihi = kModel.DRBaslamaTarihi,
                        DRMezuniyetTarihi = kModel.DRMezuniyetTarihi,
                        DRNotSistemID = kModel.DRNotSistemID,
                        DRMezuniyetNotu = kModel.DRMezuniyetNotu,
                        DRMezuniyetNotu100LukSistem = kModel.DRNotSistemID.HasValue ? kModel.DRMezuniyetNotu.Value.ToNotCevir(kModel.DRNotSistemID.Value).Not100Luk : (double?)null,
                        DREgitimDiliTurkce = kModel.YLEgitimDiliTurkce,
                        IslemTarihi = DateTime.Now,
                        IslemYapanID = UserIdentity.Current.Id,
                        IslemYapanIP = UserIdentity.Ip,
                ...
                else
                {
                    ...
                    data.DRUniversiteID = kModel.DRUniversiteID;
                    data.DRFakulteAdi = kModel.DRFakulteAdi;
                    data.DROgrenciBolumID = kModel.DROgrenciBolumID;
                    data.DRBaslamaTarihi = kModel.DRBaslamaTarihi;
                    data.DRMezuniyetTarihi = kModel.DRMezuniyetTarihi;
                    data.DRNotSistemID = kModel.DRNotSistemID;
                    data.DRMezuniyetNotu = kModel.DRMezuniyetNotu;
                    data.DREgitimDiliTurkce = kModel.DREgitimDiliTurkce;
                    if (kModel.DRNotSistemID.HasValue) data.DRMezuniyetNotu100LukSistem = kModel.DRMezuniyetNotu.Value.ToNotCevir(kModel.DRNotSistemID.Value).Not100Luk;
                    else data.DRMezuniyetNotu100LukSistem = null;
                ...
                if (sendMail && bsurec.Enstituler.LUBMailGonder)
                {
                    ...
                    {
                        mTitle = "Başvuru Sisteme Kayıt Edildi";
            ...
            ViewBag.DRUniversiteID = new SelectList(Management.cmbGetAktifUniversiteler(true), "Value", "Caption", kModel.DRUniversiteID);
            ViewBag.DROgrenciBolumID = new SelectList(Management.cmbGetOgrenciBolumleri(kModel.EnstituKod, true), "Value", "Caption", kModel.DROgrenciBolumID);
            ViewBag.DRNotSistemID = new SelectList(Management.cmbGetNotSistemleri(true), "Value", "Caption", kModel.DRNotSistemID);
            ...
            //kModel.TcKimlikNo = kul.TcKimlikNo;
            //kModel.PasaportNo = kul.PasaportNo;
            //kModel.Ad = kul.Ad;
        ...
        public ActionResult LoDKontrol(int LOgrenimDurumID, int BasvuruSurecID)
        {
            ...
            return Json(new { ShowMsg = ShowMsg, Msg = Msg, Capt = captOd });
        }
        ...
        public ActionResult getSinavTipSonuc(int SinavTipID, int BasvuruSurecID, string Donem, int? WsSonucID, string Tck, int? BasvuruID = null, string MinNotAdi = "")
        {
            ...
            //var _Donem = Donem.Split('~')[1];
            //var Donems = new List<string>() { _Donem, _Donem.Replace("0", "S") };
        ...
        {
            // ales den gre e =610 + (5 - 7/270 *(-70 + F25)) *(-55 + F25)
            // gree den ales e =55 + (1/5 + (7 *(-685 + I25))/21850) *(-610 + I25)
            // ales den gmat a =800 + (70/9 + (4/27 + 0.00415537 *(-70 + F33)) *(-55 + F33)) *(-100 + F33)
            // gmat dan ales e =100 + (9/70 + (-(1/1750) - 3.14965*(10^ -6) *(-500 + I33))* (-450 + I33))* (-800 + I33)
            string Sonuc = "";
        ...
        {
            var mmMessage = Management.getBasvuruSilKontrol(id, BasvuruSurecTipi.YatayGecisBasvuru);
