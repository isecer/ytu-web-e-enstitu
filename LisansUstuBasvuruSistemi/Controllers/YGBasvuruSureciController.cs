...
namespace LisansUstuBasvuruSistemi.Controllers
{
    ...
    [Authorize(Roles = RoleNames.YGBasvuruSureci)]
    public class YGBasvuruSureciController : Controller
    {
        private LisansustuBasvuruSistemiEntities db = new LisansustuBasvuruSistemiEntities();

        public ActionResult Index()
        {
            return Index(new fmBasvuruSureci() { PageSize = 15 });
        }
        [HttpPost]
        public ActionResult Index(fmBasvuruSureci model)
        {
            var EnstKods = UserIdentity.Current.EnstituKods ?? new List<string>();
           
            var q = from s in db.BasvuruSurecs.Where(p => p.BasvuruSurecTipID == BasvuruSurecTipi.YatayGecisBasvuru)
            ...
            var qdata = q.Skip(model.StartRowIndex).Take(model.PageSize).Select(s => new frBasvuruSureci
            {
        ...
        public ActionResult Kayit(int? id, string dlgid, string EKD)
        {
            ...
            {
                model.ToplamKota = 1;
                model.isYuzdeGirisBolum = true;
                model.Kota_BasvuruSurecKontrolTipID = 1;
                model.IsBelgeYuklemeVar = true;
                model.AnketID = 4;
            ...
            model.OgrenimTipModel.OgrenimTipleriDataList = (from o in db.OgrenimTipleris.Where(p => p.EnstituKod == _EnstituKod)
                                                            ...
                                                            select new CheckObject<krOgrenimTip>
                                                            {
                                                                Value = new krOgrenimTip
                                                                {
                                                                    ...
                                                                    BasariNotOrtalamasi = defS != null ? defS.BasariNotOrtalamasi : 1,
        ...
        public ActionResult Kayit(kmBasvuruSurec kModel, string dlgid = "")
        {
            ...
            if (MmMessage.Messages.Count == 0)
            {
                ...
                //if (kModel.BitisTarihi <= DateTime.Now)
                //{
                //    string msg = "Bitiş tarihi  şuanki tarihten küçük olamaz!.";
                //    MmMessage.Messages.Add(msg);
                //    MmMessage.MessagesDialog.Add(new MrMessage { MessageType = Msgtype.Warning, PropertyName = "BaslangicTarihi" });
                //    MmMessage.MessagesDialog.Add(new MrMessage { MessageType = Msgtype.Warning, PropertyName = "BitisTarihi" });
            ...
            if (MmMessage.Messages.Count == 0)
            {
                ...
                var qBasS = db.BasvuruSurecs.Where(p => p.BasvuruSurecTipID == BasvuruSurecTipi.YatayGecisBasvuru && p.EnstituKod == kModel.EnstituKod && p.BasvuruSurecID != kModel.BasvuruSurecID && ((p.BaslangicTarihi <= kModel.BaslangicTarihi && p.BitisTarihi >= kModel.BaslangicTarihi) || (p.BaslangicTarihi <= kModel.BitisTarihi && p.BitisTarihi >= kModel.BitisTarihi))).Count();
                if (qBasS > 0)
                {
            ...
            if (MmMessage.Messages.Count == 0)
            {
                ...
                if (kModel.BasvuruSurecID <= 0)
                {
                    ...
                    tablo = db.BasvuruSurecs.Add(new BasvuruSurec
                    {
                        ...
                        BasvuruSurecTipID = BasvuruSurecTipi.YatayGecisBasvuru,
        ...
        public ActionResult Kopyala(int basvuruSurecID, string EnstituKod, bool IsSinavOrKota)
        {
            ...
            catch (Exception ex)
            {
                ...
                Management.SistemBilgisiKaydet(ex, Msg, BilgiTipi.Hata);
            }
            mmMessage.Messages.Add(Msg);
        ...
        public ActionResult getBsDetAnaBilgi(bsUrecDetay model)
        {
            return View(model);
        }
        public ActionResult getBsSinavTipleri(bsUrecDetay model)
        {
            return View(model);
        }
        public ActionResult getBsKotalar(bsUrecDetay model)
        {
            return View(model);
        }
        public ActionResult getBsMulakat(bsUrecDetay model)
        {
            return View();
        }
        public ActionResult getBsSonuc(bsUrecDetay model)
        {
            return View();
        }
        public ActionResult getBsAnket(bsUrecDetay model)
        {
            return View();
        }
        [Authorize(Roles = RoleNames.YGBasvuruSureciKayit)]
        public ActionResult BasvuruSonucHesap(int id, string ProgramKod = null, int? OgrenimTipKod = null, int? OrtakKotaSayisi = null, int? AlanIciKota = null, int? AlanDisiKota = null, int? AlanIciEkKota = null, int? AlanDisiEkKota = null)
        {
            ...
            catch (Exception ex)
            {
                ...
                mmMessage.Messages.Add(msg);
                Management.SistemBilgisiKaydet(msg, "BasvuruSureci/BasvuruSonucHesap<br/><br/>" + ex.ToExceptionStackTrace(), BilgiTipi.Kritik);
            }
        ...
        [Authorize(Roles = RoleNames.YGBasvuruSureciSil)]
        public ActionResult Sil(int id)
        {
            ...
            if (kayit != null)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Management.SistemBilgisiKaydet(message, "YGBasvuruSureci/Sil<br/><br/>" + ex.ToExceptionStackTrace(), BilgiTipi.OnemsizHata);
        ...
        public ActionResult mailGonderJuriYer(int id, bool JuriBilgisiGonderilsin = false)
        {
            ...
            try
            {
                Management.SistemBilgisiKaydet(SurecAdiB + " Başvuru sürecine ait Mülakat Sınav yeri ve jüri bilgisi mail gönderim işlemi başlatıldı!", "BasvuruSureci/mailGonderJuriYer", BilgiTipi.Bilgi);
                ...
                var qMulakats = (from s in db.Mulakats
                                 ...
                                 where s.BasvuruSurecID == id
                                 select new mailMulakatSinavYerJuriBilgi
                                 {
                                     MulakatID = s.MulakatID,
                                     BasvuruSurecID = id,
                                     OgrenimTipKod = s.OgrenimTipKod,
                ...
                foreach (var item in qMulakats)
                {
                    if (item.GonderilecekMails.Count > 0)
                    {
                        ...
                        mtc.Detaylar.Add(new mailTableRow { Baslik = "Başvuru Dönemi", Aciklama = SurecAdi });
                        mtc.Detaylar.Add(new mailTableRow { Baslik = "Anabilim Dalı / Program", Aciklama = (item.AnabilimDaliAdi + " / " + item.ProgramAdi) });
                        mtc.Detaylar.Add(new mailTableRow { Baslik = "Öğrenim Seviyesi", Aciklama = item.OgrenimTipAdi });
                        var mdHtml = new List<string>();
                        foreach (var itmMD in item.MulakatDetayB)
                        {
                            var mtcSinavB = new mailTableContent();
                            mtcSinavB.GrupBasligi = itmMD.MulakatSinavTurAdi;
                            mtcSinavB.Detaylar.Add(new mailTableRow { Baslik = "Kampüs", Aciklama = itmMD.KampusAdi });
                        ...
                        if (JuriBilgisiGonderilsin)
                        {
                            ...
                            {
                                mtcSinavJ.Detaylar.Add(new mailTableRow { SiraNo = itemJr.SiraNo, Baslik = itemJr.JuriAdi, Aciklama = itemJr.IsAsil ? "Asil" : "Yedek" });
                            }
                            mtc.Detaylar.Add(new mailTableRow
                            {
                                Colspan2 = true,
            ...
            catch (Exception ex)
            {
                ...
                Management.SistemBilgisiKaydet(title + "\r\n Hata:" + ex.ToExceptionMessage(), "BasvuruSureci/mailGonderJuriYer", BilgiTipi.Hata);
        ...
        public ActionResult mailGonderBolumSinavYerBilgiGiris(int id, bool isSinavYerOrSinavNotGiris)
        {
            ...
            Management.SistemBilgisiKaydet(SurecAdiB + " Başvuru sürecine ait Mülakat " + (isSinavYerOrSinavNotGiris ? "Sınav yer ve Jüri bilgileri" : "Sınav Notu") + " girişi için Anabilim Dallarına bilgi maili gönderim işlemi başlatıldı!", "BasvuruSureci/mailGonderBolumSinavYerBilgiGiris", BilgiTipi.Bilgi);
            try
            {
                var rollers = new List<string> { RoleNames.YGMulakatSureci, RoleNames.YGMulakatKayıt };
                var qData = (from s in db.BasvuruSurecKotalars.Where(p => p.BasvuruSurecID == id)
                             ...
                             join otAd in db.OgrenimTipleris on otOr.OgrenimTipID equals otAd.OgrenimTipID
                             ...
                             select new mailMulakatSinavYerJuriBilgiBolum
                             {
                                 AnabilimDaliKod = g1.Key.AnabilimDaliKod,
                                 AnabilimDaliAdi = g1.Key.AnabilimDaliAdi,
                                 ...
                                 detay = g1.Where(p => p.AnabilimDaliKod == g1.Key.AnabilimDaliKod).Select(s => new mailMulakatSinavYerJuriBilgiBolumDetay
                                 {
                ...
                var qDataList = new List<mailMulakatSinavYerJuriBilgiBolum>();
                if (isSinavYerOrSinavNotGiris)
                {
                    qDataList = qData.Where(p => p.detay.Any(a => a.EksikBilgiSinavYerBilgi)).ToList();
                }
                else
                {
                    ...
                    {
                        var mdetay = new List<mailMulakatSinavYerJuriBilgiBolumDetay>();
                        foreach (var item2 in item.detay)
                        {
                    ...
                    qDataList = qDataList.Where(p => p.detay.Count > 0).ToList();
                ...
                foreach (var item in qDataList)
                {
                    ...
                    mtc.Detaylar.Add(new mailTableRow { Baslik = "Başvuru Dönem Adı", Aciklama = SurecAdiB });
                    mtc.Detaylar.Add(new mailTableRow { Baslik = "Anabilim Dalı Adı", Aciklama = item.AnabilimDaliAdi });
                    var qdetay = item.detay.AsQueryable();
                    if (isSinavYerOrSinavNotGiris)
                    {
                        qdetay = qdetay.Where(a => a.EksikBilgiSinavYerBilgi);
                        mtc.Detaylar.Add(new mailTableRow { Baslik = "Mülakat Bilgi Giriş Süreci", Aciklama = SinavBilgiGirisSurec });
                        mtc.Detaylar.Add(new mailTableRow { Baslik = "Bilgi Girişi Kalan Süre", Aciklama = SinavBilgiGirisSureckalanGun == 0 ? "Bu gün bilgi girişi için son gün" : (SinavBilgiGirisSureckalanGun + " Gün") });
                        mtc.Detaylar.Add(new mailTableRow { Colspan2 = true, Aciklama = "Aşağıda bulunan programlara ait Sınav giriş ve Jüri bilgileri " + bsurec.BitisTarihi.ToFormatDateAndTime() + " Tarihine kadar eksiksiz bir şekilde girilmesi gerekmektedir." });
                    }
                    else
                    {

                        mtc.Detaylar.Add(new mailTableRow { Baslik = "Mülakat Not Giriş Süreci", Aciklama = SinavNotGirisSurec });
                        mtc.Detaylar.Add(new mailTableRow { Baslik = "Not Girişi İçin Kalan Süre", Aciklama = SinavNotGirisSureckalanGun == 0 ? "Bu gün not girişi için son gün" : (SinavNotGirisSureckalanGun + " Gün") });
                        mtc.Detaylar.Add(new mailTableRow { Colspan2 = true, Aciklama = "Aşağıda bulunan programlara ait Sınav notlarının " + bsurec.SonucGirisBitisTarihi.Value.ToFormatDateAndTime() + " Tarihine kadar eksiksiz bir şekilde girilmesi gerekmektedir." });
                    }
                    var mdHtml = new List<string>();
                    ...
                    foreach (var itemD in qdetayList)
                    {
                        ...
                        mtcSinavB.Detaylar.Add(new mailTableRow { Baslik = "Program Adı", Aciklama = itemD.ProgramAdi });
                        mtcSinavB.Detaylar.Add(new mailTableRow { Baslik = "Öğrenim Seviyesi", Aciklama = itemD.OgrenimTipAdi });
                        mtcSinavB.Detaylar.Add(new mailTableRow { Baslik = "Alaniçi Kota", Aciklama = itemD.AlaniciKota.ToString() });
                        mtcSinavB.Detaylar.Add(new mailTableRow { Baslik = "Alandışı Kota", Aciklama = itemD.AlandisiKota.ToString() });
            ...
            catch (Exception ex)
            {
                ...
                Management.SistemBilgisiKaydet(message + "\r\n Hata:" + ex.ToExceptionMessage(), "BasvuruSureci/mailGonderBolumSinavYerBilgiGiris", BilgiTipi.Hata);
                mmMessage.Title = "Hata";
        ...
        public ActionResult basvuruSonucMailGonderim(int id, bool IsSonucOrAnket, string OgrenimTipKods)
        {
            ...
            if (mmMessage.Messages.Count == 0)
            {
                Management.SistemBilgisiKaydet(SurecAdiB + " Başvuru sürecine ait başvuru " + (IsSonucOrAnket ? "sonuçları" : "Anket giriş bilgileri") + " öğrencilere gönderilmeye başlandı!", "BasvuruSureci/basvuruSonucMailGonderim", BilgiTipi.Bilgi);
                try
                {
                    var Bsonuc = (from s in db.MulakatSonuclaris.Where(p => p.BasvuruSurecID == id && (!IsSonucOrAnket ? (p.KayitDurumID.HasValue && p.KayitDurumlari.IsKayitOldu != true && p.MulakatSonucTipID == MulakatSonucTipi.Asil) : 1 == 1))
                                  ...
                                  join b in db.Basvurulars.Where(p => (!IsSonucOrAnket ? !p.AnketCevaplaris.Any() : true)) on bt.BasvuruID equals b.BasvuruID
                                  join mst in db.MulakatSonucTipleris on s.MulakatSonucTipID equals mst.MulakatSonucTipID
                                  join bs in db.BasvuruSurecs on b.BasvuruSurecID equals bs.BasvuruSurecID
                                  join bd in db.Donemlers on bs.DonemID equals bd.DonemID
                                  join pr in db.Programlars on bt.ProgramKod equals pr.ProgramKod
                                  join bl in db.AnabilimDallaris on new { bs.EnstituKod, pr.AnabilimDaliKod } equals new { bl.EnstituKod, bl.AnabilimDaliKod }
                                  ...
                                  select new mailBsonucModel
                                  {
                                      ...
                                      AnabilimDaliAdi = bl.AnabilimDaliAdi,
                                      ProgramAdi = pr.ProgramAdi,
                    ...
                    var enstituAdi = db.Enstitulers.Where(p => p.EnstituKod == bsurec.EnstituKod).First();
                    ...
                    foreach (var item in Bsonuc)
                    {
                        ...
                        mmmC.EnstituAdi = enstituAdi.EnstituAd;
                        mmmC.UniversiteAdi = "Yıldız Teknik Üniversitesi";
                ...
                catch (Exception ex)
                {
                    ...
                    Management.SistemBilgisiKaydet(message + "\r\n Hata:" + ex.ToExceptionMessage(), "BasvuruSureci/basvuruSonucMailGonderim", BilgiTipi.Hata);
                    mmMessage.Title = "Hata";
                    mmMessage.Messages.Add(message + "</br> Hata:" + ex.ToExceptionMessage());
                    mmMessage.MessageType = Msgtype.Error;
                    mmMessage.IsSuccess = false;
        ...
        [Authorize(Roles = (RoleNames.YGBasvuruSureciOgrenciKayit))]
        public ActionResult KazananKayit(int MulakatSonucID, int? KayitDurumID, bool YokODKontrolYap = true, bool? IsSave = null)
        {

            var mmMessage = new MmMessage();
            mmMessage.IsSuccess = true;
            mmMessage.Title = "Yatay Geçiş Başvuru Sonucu Kesin Kayıt İşlemi";
            ...
            var ProgramBilgi = Management.GetOnlineOdemeProgramDetay(btercih.BasvurularTercihleri.UniqueID.ToString() ,false, false, YokODKontrolYap);
            ...
            var ogrenciGSISBilgi = Management.StudentControl(null,ProgramBilgi.TcKimlikNo);
            ...
            if (mmMessage.IsSuccess)
            {
                ...
                if (!ogrenciGSISBilgi.Hata)
                {
                    if (ogrenciGSISBilgi.KayitVar)
                    {
                        var programKod = ogrenciGSISBilgi.OgrenciInfo.PROGRAM_ID;
                        ...
                        if ((_enstituKod == ProgramBilgi.EnstituKod) && KayitDurumID.HasValue && KayitDurumID == KayitDurumu.KayitOldu)
                        {
                            ...
                            mmMessage.Messages.Add(msg);
                            mmMessage.IsSuccess = false;
                            Management.SistemBilgisiKaydet(msg, "BasvuruSureci/KazananKayit", BilgiTipi.Uyarı);
                        }
                ...
                {
                    var msg = OgrenciBilgi + " si için GSIS sisteminde kayıt kontrolü başarısız oldu! Lütfen sistem yöneticisine başvurunuz!";
                    mmMessage.Messages.Add(msg);
                    mmMessage.IsSuccess = false;
                    Management.SistemBilgisiKaydet(msg, "BasvuruSureci/KazananKayit", BilgiTipi.Kritik);
                }
            }
            ...
            if (mmMessage.IsSuccess)
            {
                ...
                if (KayitDurumID == KayitDurumu.KayitOldu)
                {
                    ...
                    else
                    {
                        ...
                        Management.SistemBilgisiKaydet(OgrenciBilgi + "nin Kayıt işlemi sırasında bir hata oluştu! GSIS sistemi hata döndürdü! <br/> Mesaj:" + returnVal + " <br/><br/>" + sw, "BasvuruSureci/KazananKayit", BilgiTipi.Kritik);
                        mmMessage.IsSuccess = false;
                    }
                    IsReloadPage = true;
                }
                ...
                #region SendMail
                if (KayitDurumID == KayitDurumu.KayitOldu || (KayitDurumID == KayitDurumu.OnKayit && ProgramBilgi.IsOgrenimUcretiOrKatkiPayi == false))
                {
                    ...
                    var MailSablonTipID = KayitDurumID == KayitDurumu.KayitOldu ? MailSablonTipi.OtoMailKayitOldu : MailSablonTipi.OtoMailOnKayitOldu;
                    ...
                    if (Sablon != null)
                    {
                        ...
                        if (Parametreler.Any(a => a == "@WebAdresi"))
                            ParamereDegerleri.Add(new MailReplaceParameterModel { Key = "WebAdresi", Value = EnstituL.WebAdresi, IsLink = true });
                        if (Parametreler.Any(a => a == "@AdSoyad"))
                            ParamereDegerleri.Add(new MailReplaceParameterModel { Key = "AdSoyad", Value = Basvuru.Ad + " " + Basvuru.Soyad });

                        if (Parametreler.Any(a => a == "@ProgramAdi"))
                            ParamereDegerleri.Add(new MailReplaceParameterModel { Key = "ProgramAdi", Value = ProgramBilgi.ProgramAdi });
                        if (Parametreler.Any(a => a == "@OgrenciNo"))
                            ParamereDegerleri.Add(new MailReplaceParameterModel { Key = "OgrenciNo", Value = OgrenciNo });
                        if (Parametreler.Any(a => a == "@OgrenimSeviyesiAdi"))
                            ParamereDegerleri.Add(new MailReplaceParameterModel { Key = "OgrenimSeviyesiAdi", Value = ProgramBilgi.OgrenimTipAdi });
        ...
        public ActionResult getImages(int id)
        {
            ...
            {
                var bs = db.BasvuruSurecs.Where(p => p.BasvuruSurecID == id).First();
                var kayitOlanlar = db.MulakatSonuclaris.Where(p => p.BasvuruSurecID == id && p.KayitDurumID.HasValue && p.KayitDurumlari.IsKayitOldu == true).Select(s => new { s.Basvurular.Kullanicilar.OgrenciNo, s.BasvurularTercihleri.Basvurular.ResimAdi }).Distinct().ToList();
                ...
                using (ZipFile zipFile = new ZipFile())
                {
                    ...
                    {
                        var Resim = Images.Where(p => p.Value == item.ResimAdi).FirstOrDefault();
                        if (Resim != null)
                        {
                            ...
                            using (MemoryStream tempstream = new MemoryStream())
                            {
                                ...
                                var fileName = item.OgrenciNo + ".jpeg";
