...
{
    public partial class rprBasvuruDoktora : DevExpress.XtraReports.UI.XtraReport
    {
        public rprBasvuruDoktora(int id)
        {
            InitializeComponent();
            using (var db = new LisansustuBasvuruSistemiEntities())
            {
                int? KullaniciID = RoleNames.GelenBasvurular.InRoleCurrent() ? (int?)null : UserIdentity.Current.Id;
                var Basvuru = db.BasvurularTercihleris.Where(p => p.BasvuruTercihID == id && (KullaniciID.HasValue ? p.Basvurular.KullaniciID == KullaniciID : 1 == 1)).First();
                ...
                cell_ProgramAdi.Text = btercih.ProgramBilgileri.ProgramAdi;
                lngLbl_SinavBilgileri.Text = "SINAV BİLGİLERİ";
                    ...
                    if (model.BasvurularSinavBilgi_A.SinavDetay.NotDonusum || model.BasvurularSinavBilgi_A.SinavDetay.OzelNotTipID == OzelNotTip.SeciliNotAraliklari)
                    {
                        ...
                        if (model.BasvurularSinavBilgi_A.Sinav.SubSinavAralikID.HasValue)
                        {
                            ...
                            if (sclnKriter.NotDonusum)
                            {
                                ...
                                notB = model.BasvurularSinavBilgi_A.Sinav.BasvuruSurecSubNot + " (Ales karşılığu:" + model.BasvurularSinavBilgi_A.Sinav.SinavNotu + ")";
                            }
                        ...
                        cell_SinavPuan.Text = model.BasvurularSinavBilgi_A.Sinav.BasvuruSurecSubNot.ToString();
                    }
                    else
                    {
                ...
                // cell_SinavTarih.Text = DateTime.Now.ToDateString(); // model.BasvurularSinavBilgi_A.Sinav.SinavTarihi.Value.ToDateString();
                ...
                lngLbl_LAGNO.Text = "AGNO (YÖK Karşılığı)";
                cell_LAGNO.Text = model.LMezuniyetNotu.Value.ToNotCevir(model.LNotSistemID.Value).Not100Luk.ToString("n2");
                ...
                if (model.DRUniversiteID.HasValue)
                {
                    SubBand2.Visible = true;
                    lblCaptDrEgitimi.Text = "Doktora Eğitimi";
                    lblDrUniAdi.Text = "Üniversite";
                    cellDrUniAdi.Text = model.DRUniversiteAdi;
                    lblDrFakulte.Text = "Enstitü";
                    cellDrFakulte.Text = model.DRFakulteAdi;
                    lblDrBolum.Text = "Bölüm";
                    cellDrBolum.Text = model.DRBolumAdi;
                    lblDrAgno.Text = "AGNO";
                    cellDrAgno.Text = "Puan Sistemi:" + model.DRNotSistemi + ", Not:" + model.DRMezuniyetNotu.Value.ToString("n2");
                    lblDrAgnoYok.Text = "AGNO (YÖK Karşılığı)";
                    cellDrAgnoYok.Text = model.DRMezuniyetNotu.Value.ToNotCevir(model.DRNotSistemID.Value).Not100Luk.ToString("n2");
                }
