                ...
                using (var entities = new LubsDbEntities())
                {
                    ...
                    if (eykDaOnayOrEykYaGonderim)
                    {
                        if (onaylandi)
                        {
                            ...
                            var jurlerYeni = tijBasvuruOneri.TijBasvuruOneriJurilers.Where(p => p.IsYeniOrOnceki && p.IsAsil == true && !p.IsTezDanismani).OrderBy(o => o.RowNum).ToList();
                            mModel.AddRange(jurlerYeni.Select(item => new SablonMailModel
                            {
                            ...
                            var jurlerEski = tijBasvuruOneri.TijBasvuruOneriJurilers.Where(p =>
                                 (tijBasvuruOneri.TijDegisiklikTipID == TijDegisiklikTipiEnum.YtuIciVeDisiDegisiklik
                                   || p.IsYtuIciJuri == (tijBasvuruOneri.TijDegisiklikTipID == TijDegisiklikTipiEnum.YtuIciDegisiklik)
                                   || !p.IsYtuIciJuri == (tijBasvuruOneri.TijDegisiklikTipID == TijDegisiklikTipiEnum.YtuDisiDegisiklik))
                                  && !p.IsYeniOrOnceki && !p.IsTezDanismani).OrderBy(o => o.RowNum).ToList();
                            mModel.AddRange(jurlerEski.Select(item => new SablonMailModel
                            {
                                JuriTipAdi = "Eski Jüri Üyesi",
                                UnvanAdi = item.UnvanAdi,
                                AdSoyad = item.AdSoyad,
                                EMails = new List<MailSendList> { new MailSendList { EMail = item.EMail, ToOrBcc = true } },
                                MailSablonTipId = MailSablonTipiEnum.TijOneriFormuEykdaOnaylandiEskiJuriUyelerine
                            }));
                        }
                        mModel.Add(
                            new SablonMailModel
                    ...
                    {
                        var tumJuriler = tijBasvuruOneri.TijBasvuruOneriJurilers.ToList();
                        juriUyeleri = new List<TijBasvuruOneriJuriler> { tumJuriler.First(f => f.IsTezDanismani) };
                        //Yeni asil jürileri jüri listesine ekle
                        juriUyeleri.AddRange(tumJuriler.Where(p => p.IsYeniOrOnceki && p.IsAsil == true));
                        var oncekiJuriler = tumJuriler.Where(p => !p.IsYeniOrOnceki && !p.IsTezDanismani).ToList();
                        foreach (var oncekiJuri in oncekiJuriler)
                        {
                            // jüri değişmediyse jüri listesine ekle
                            if (!juriUyeleri.Any(a => !a.IsTezDanismani && a.IsYtuIciJuri == oncekiJuri.IsYtuIciJuri))
                                juriUyeleri.Add(oncekiJuri);
                        }
                        juriUyeleri = juriUyeleri.OrderBy(o => o.RowNum).ToList();
                    }
                    ...
                    foreach (var item in mModel)
                    {
                        ...
                        if (juriUyeleri != null)
                        {
                            ...
                            var tezDanisman = juriUyeleri.First(p => p.IsTezDanismani);
                            if (item.SablonParametreleri.Any(a => a == "@DanismanBilgi"))
                                item.MailParameterDtos.Add(new MailParameterDto { Key = "DanismanBilgi", Value = tezDanisman.UnvanAdi + " " + tezDanisman.AdSoyad });
